...
void md5transform(uint32 buf[], const unsigned char in[]);
 ...
 * of bytes.
 */
void md5update(t_md5 *md5data, unsigned char const *buf, unsigned len)
{
	uint32 t;
	...
	t = md5data->bits[0];
	if ((md5data->bits[0] = (t + ((uint32)len << 3)) & 0xffffffff) < t)
		md5data->bits[1]++;	/* Carry from low to high */
...
void md5final(unsigned char digest[], t_md5 *md5data)
{
	...
	*p++ = 0x80;
	...
	memset(md5data, 0, sizeof(md5data));	/* In case it's sensitive */
}
 ...
 */
void md5transform(uint32 buf[], const unsigned char inraw[])
{
	uint32 a, b, c, d;
	uint32 in[16];
	int i;
